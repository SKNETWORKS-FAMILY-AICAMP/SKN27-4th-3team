...
- `피티`를 `nameless_curse` 입력 없이 끌어온다.
- 데모 스토리 참고 문서의 `이안`, `엘리자베스`, `무명(無名)의 저주`를 입력 없이 끌어온다.
...
- 판정처럼 보이는 `드러났다`, `밝혀졌다`, `제공` 표현을 공개 로그 근거 없이 사용한다.
- 10자 미만 또는 60자 초과로 출력한다.
...
- 괴이 반응은 노골적 협박보다 거울, 침묵, 이름, 시선 같은 감각 이미지로 압박한다.
