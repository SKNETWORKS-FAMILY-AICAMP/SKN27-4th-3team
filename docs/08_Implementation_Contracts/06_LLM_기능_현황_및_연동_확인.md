...
현재는 `피티`를 `nameless_curse`의 공식 괴이 표시명/진명으로 취급한다.

LLM은 `nameless_curse` payload에 `피티`가 들어온 경우에만 피티를 공식 이름으로 사용할 수 있다.

`이안`, `엘리자베스`, 입력에 없는 `무명(無名)의 저주`는 이전 초고 흔적으로 취급하며, 입력 payload에 없으면 생성 결과에 끌어오면 안 된다.
...
- `무명(無名)의 저주`는 [[09_Approved_Contracts/26_무명의_저주_사건_계약]]에 명시된 seed 범위에서만 공식 사건 설정으로 사용한다.
